#!/usr/bin/env python3
"""Bootstrap curated Finnish FMI air-quality CSAPI resources."""
...
PROC_UID = "urn:os4csapi:procedure:fmi-air-quality:v1"
DEPLOY_ROOT_UID = "urn:os4csapi:deployment:fmi-air-quality-demo:v1"
DEPLOY_GROUP_UID = "urn:os4csapi:deployment:fmi-air-quality-stations:v1"
...
def _deploy_uid(station_id: str) -> str:
    return f"urn:os4csapi:deployment:fmi-air-quality-{_uid_token(station_id)}:v1"
...
def _system_stub(station: dict) -> dict:
    return {"type": "Feature", "geometry": {"type": "Point", "coordinates": [station["lon"], station["lat"]]}, "properties": {"uid": _system_uid(station["stationId"]), "featureType": "sosa:Sensor", "name": f"FMI Air Quality {station['name']}", "description": f"Curated FMI air-quality monitoring location for {station['name']}.", "validTime": [VALID_TIME_START, ".."]}}
...
def _deploy_root() -> dict:
    return {"type": "Feature", "geometry": {"type": "Point", "coordinates": [25.0, 63.5]}, "properties": {"uid": DEPLOY_ROOT_UID, "featureType": "sosa:Deployment", "name": "FMI Air Quality Demo", "description": "Top-level grouping for curated FMI air-quality resources.", "validTime": [VALID_TIME_START, ".."]}}


def _deploy_group() -> dict:
    return {"type": "Feature", "geometry": {"type": "Point", "coordinates": [25.0, 63.5]}, "properties": {"uid": DEPLOY_GROUP_UID, "featureType": "sosa:Deployment", "name": "FMI Air Quality Stations", "description": "Grouping deployment for curated FMI air-quality monitoring locations.", "validTime": [VALID_TIME_START, ".."]}}
...
def _ensure_system_resilient(base_url: str, auth: str, station: dict, *, dry_run: bool, stats: dict, force_sml: bool) -> str | None:
    ...
    try:
        ...
        if "HTTP 500 POST" not in str(exc) or "/systems" not in str(exc): raise
        ...
        if not dry_run:
            try: api_put(base_url, f"systems/{recovered}", _system_sml(station), auth, content_type="application/sml+json")
            except Exception as sml_exc: print(f"  [WARN] SML PUT skipped for recovered system {uid}: {sml_exc}")
        stats["recovered"] = stats.get("recovered", 0) + 1
        return recovered


def clean_all(base_url: str, auth: str, *, dry_run: bool, stats: dict):
    for station in _load_stations(): clean_resource(base_url, auth, "deployments", _deploy_uid(station["stationId"]), dry_run=dry_run, stats=stats, cascade=True)
    clean_resource(base_url, auth, "deployments", DEPLOY_GROUP_UID, dry_run=dry_run, stats=stats, cascade=True); clean_resource(base_url, auth, "deployments", DEPLOY_ROOT_UID, dry_run=dry_run, stats=stats, cascade=True)
    for station in _load_stations(): clean_resource(base_url, auth, "systems", _system_uid(station["stationId"]), dry_run=dry_run, stats=stats, cascade=True)
...
def bootstrap(*, clean: bool = False, clean_only: bool = False, dry_run: bool = False, force_sml: bool = False):
    ...
    if clean or clean_only:
        ...
        if clean_only: print_summary(stats, dry_run); return
    ...
    for station in stations: ensure_deployment(base_url, auth, _deploy_uid(station["stationId"]), _deploy_station(station, system_ids.get(station["stationId"], "pending"), base_url), parent_id=group_id, dry_run=dry_run, stats=stats)
    print_summary(stats, dry_run)


def main():
    parser = argparse.ArgumentParser(description="Bootstrap FMI air-quality resources on the CSAPI server."); add_bootstrap_args(parser); args = parser.parse_args(); bootstrap(clean=args.clean, clean_only=args.clean_only, dry_run=args.dry_run, force_sml=args.force_sml)
